...
public final class ConvexHull {
    ...
    public static List<HullPoint> makeHull(List<HullPoint> points) {
        List<HullPoint> newPoints = new ArrayList<>(points);
    ...
    public static List<HullPoint> makeHullPresorted(List<HullPoint> points) {
        ...
        for (HullPoint p : points) {
            while (upperHull.size() >= 2) {
                HullPoint q = upperHull.get(upperHull.size() - 1);
                HullPoint r = upperHull.get(upperHull.size() - 2);
        ...
        for (int i = points.size() - 1; i >= 0; i--) {
            HullPoint p = points.get(i);
            while (lowerHull.size() >= 2) {
                HullPoint q = lowerHull.get(lowerHull.size() - 1);
                HullPoint r = lowerHull.get(lowerHull.size() - 2);
                if ((q.x - r.x) * (p.y - r.y) >= (q.y - r.y) * (p.x - r.x))
